...
public class EventBusLauncher {
    ...
    /*
        Problem 1:
            ...
            How to process requests in order for each user? Could be a class Node containing an AtomicBoolean and a Queue as a value of a Map<UserId, Node>. The boolean indicates only one Thread at a time is processing one user's requests. (Look at Load Balancer mechanism)
        ...
        Problem 2:
            We have an array of ascending numbers and then of descending numbers. Find the pivot? Could use the binary search from the middle and check if leftV < midV look on the right partition else on the left partition (log n time).
     */
